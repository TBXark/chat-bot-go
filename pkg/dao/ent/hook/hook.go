...
func (f ChatHistoryFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.ChatHistoryMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.ChatHistoryMutation", m)
}

// The OpenAITokenFunc type is an adapter to allow the use of ordinary
// function as OpenAIToken mutator.
type OpenAITokenFunc func(context.Context, *ent.OpenAITokenMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f OpenAITokenFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.OpenAITokenMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.OpenAITokenMutation", m)
}
